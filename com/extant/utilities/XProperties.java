...
extends Properties
{
    public final static String defaultPropertiesFilename = "E:\\ACCOUNTING\\EXTANT\\EXTANT.properties";
    ...
    public XProperties( String propertiesFilename )
    throws IOException, FileNotFoundException
    {
        this( propertiesFilename, "" );
    }

    public XProperties( String propertiesFilename, String section )
    throws IOException, FileNotFoundException
    {
        if ( propertiesFilename == null ) throw new IOException( "propertiesFilename is null!" );
...
//
    ...
    {
        String realKey;
        if ( super.containsKey( key ) ) realKey = key;
        else realKey = realKey( key );
    ...
    {
        String v = "Double|";
        if ( format == null ) v += Double.toString( value );
        else
    ...
    public Point getPoint( String key, Point defaultPoint )
    throws UtilitiesException
    {
        String realKey = realKey( key );
        if ( super.containsKey( realKey ) )
        {
            ...
            if ( !parseClass( v ).toLowerCase().endsWith( "point" ) ) throw new UtilitiesException
                ( UtilitiesException.PROPERTY_TYPE_MISMATCH, key + "=" + v );
    ...
    public Dimension getDimension( String key, Dimension defaultDimension )
    throws UtilitiesException
    {
        String realKey = realKey( key );
        if ( super.containsKey( realKey ) )
        {
            String v = super.getProperty( realKey( key ) );
            if ( !parseClass( v ).toLowerCase().endsWith( "dimension" ) ) throw new UtilitiesException
                ( UtilitiesException.PROPERTY_TYPE_MISMATCH, key + "=" + v );
            return new Object2D( v ).toDimension();
        }
        else return defaultDimension;
    }
    ...
    {
        if ( !super.containsKey( realKey( key ) ) ) return false;
    ...
    throws IOException
    {
        if ( propertiesHaveChanged ) this.store( propertiesFilename, comments );
    }

    // this will force an update of the specified properties file
    public void store( String outfilename, String comments )
    throws IOException
    {
...
//    }
    ...
    {
        if ( section.equals( "" ) ) return key;
        else return section + "." + key;
    }
...
//Console.println( "validating: " + key + "=" + value );
            ...
            try
            {
                if ( clazz.endsWith( "string" ) ) ;  // All Strings are good
                else if ( clazz.endsWith( "point" ) ) new Object2D( value );
                else if ( clazz.endsWith( "int" ) ) Integer.parseInt( v );
                else if ( clazz.endsWith( "double" ) ) Double.parseDouble( v );
                else if ( clazz.endsWith( "dimension" ) ) new Object2D( value );
    ...
    public static void main( String args[] )
    {
        try
        {
            XProperties xProperties = new XProperties
                ( "G:\\Projects\\Java7\\EXTANT\\Extant.properties", "TEST" );
            int nErrors = xProperties.validate();
            Console.println( "validate reports " + Strings.plurals( "error", nErrors ) );
            if ( nErrors > 0 ) Console.println( xProperties.getReport() );

            // This is a simple string
            Console.println( "transporter.archive=" +
                xProperties.getString( "transporter.archive" ) );
            // This is a string with some "\\" characters
            Console.println( "transporterLogFilename=" +
                xProperties.getString( "transporterLogFilename" ) );
            // This is a directory name (with trailing "\\")
            Console.println( "transporterDir=" +
                xProperties.getString( "transporterDir" ) );
            // This is a type int
            Console.println( "DBMaint.COINDEX.CONAME.width=" +
                xProperties.getInt( "DBMaint.COINDEX.CONAME.width" ) );
            // This is a String with an integer value
            Console.println( "DBMaint.COINDEX.CUSIP.width=" +
                xProperties.getString( "DBMaint.COINDEX.CUSIP.width" ) );
            Console.println( "DBMaint.COINDEX.CUSIP.width=" +
                xProperties.getInt( "DBMaint.COINDEX.CUSIP.width" ) );
            // This is a Point
            Point point = xProperties.getPoint( "dbTools.ReportViewer.StockTrans.frameLocation" );
            Console.print( "dbTools.ReportViewer.StockTrans.frameLocation=" );
            if ( point == null ) Console.println( "null" );
            else Console.println( point.toString() );
            // This is a Dimension
            Dimension dim = xProperties.getDimension( "dbTools.ReportViewer.StockTrans.frameSize" );
            Console.print( "dbTools.ReportViewer.StockTrans.frameSize=" );
            if ( dim == null ) Console.println( "null" );
            else Console.println( dim.toString() );
            // Now we put some new things in
            xProperties.putString( "TestString", "This is a test string." );
            Console.println( "TestString=" + xProperties.getString( "TestString" ) );
            xProperties.putInt( "TestInt", 77 );
            Console.println( "TestInt=" + xProperties.getInt( "TestInt" ) );
            xProperties.putPoint( "TestPoint", new Point( 13, 17 ) );
            Console.println( "TestPoint=" + xProperties.getPoint( "TestPoint" ).toString() );
            xProperties.putDimension( "TestDimension", new Dimension( 23, 27 ) );
            Console.println( "TestDimension=" + xProperties.getDimension( "TestDimension" ).toString() );

            String storeFile = "C:\\Projects\\xPropTest2.properties";
            Console.println( "storing to " + storeFile + " ..." );
            xProperties.store( storeFile, "Testing 1 2 3" );

            Console.println( "This should cause a PropertyTypeMismatch exception:" );
            point = xProperties.getPoint( "TestDimension" );
            if (point == null) Console.println( null );
    ...
    {
        XProperties props;
        try { props = new XProperties("G:\\ACCOUNTING\\JMS\\JMS.properties"); }
        catch (IOException iox) {System.out.println("Cannot open JMS.properties"); return;}
