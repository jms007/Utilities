...
{
    public ViewFile( String fileName, LogFile logger )
        throws UtilitiesException
    {
        this( fileName, logger, false );
    }

    public ViewFile( String fileName, LogFile logger, boolean standalone )
    throws UtilitiesException
    {
    ...
    private void view()
    {
        try
        {
            ...
            {   // Use our text file viewer (which does not allow editing)
                ...
                while ( !file.EOF() ) text.append( file.readLine( UsefulFile.RIGHT_WHITE ) + "\n" );
                file.close();
        ...
        catch (IOException iox) { logger.log( logger.FATAL_ERROR, iox.getMessage() ); }
        catch (InterruptedException ix) { logger.log( logger.FATAL_ERROR, ix.getMessage() ); }
        catch (UtilitiesException ux) { logger.log( logger.FATAL_ERROR, ux.getMessage() ); }
        exitForm( null );
    }
...
//    { new DOSExec( "\"C:\\Program Files\\Microsoft Office\\Office\\excel.exe\" \"" + filename + "\"" ); }
//
    /**
    ...
    public static void main(String args[])
    {
        try
        {
            new ViewFile( "E:ACCOUNTING\\EXTANT\\GL17\\CHART.PDF", new LogFile(), true ).setVisible( true );
            return;
        }
