...
public class TextReportFormatter extends javax.swing.JFrame
{
    ...
    private void initComponents() {
        ...
        jPanel1.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Input File", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Arial", 0, 10))); // NOI18N
        ...
        jPanel2.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Output File", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Arial", 0, 10))); // NOI18N
    ...
    {//GEN-HEADEREND:event_btnOKActionPerformed
        ...
        catch (UtilitiesException ux) { statusBar.setText( ux.getMessage() ); }
    ...
    throws IOException
    {
        ...
        for (int i=0; i<delimiters.length; i++ )
        {
            ...
            if ( t > maxCount) { maxCount = t; maxChar = delimiters[i]; }
        }
        char da[] = new char[1];
        da[0] = maxChar;
        txtDelimiter.setText( new String( da ) );
        txtNoColumns.setText( new Integer(maxCount + 1 ).toString() );
        //inputFile.rewind();  we exclude the first line when computing column widths & types
        colWidths = new int[maxCount + 1];
        colTypes = new int[maxCount + 1];
        for (int i=0; i<=maxCount; ++i) { colWidths[i] = 0; colTypes[i] = RIGHT; }
        int lineNo = 0;
        while ( !inputFile.EOF() )
        {
            image = inputFile.readLine( UsefulFile.EOL );
            ...
            while( st.hasMoreElements() )
            {
                ...
                if ( entry.length() > colWidths[col] ) colWidths[col] = entry.length();
                if ( !isNumber( entry ) && colTypes[col] == RIGHT ) colTypes[col] = LEFT;
                ++col;
            }
//if ( col != maxCount+1 ) Console.println( "col=" + col + "  maxCount=" + maxCount +
//  " in Line " + lineNo );
        }
...
//Console.println( "colTypes[" + i + "]=" + colTypes[i] );
    ...
    throws IOException, UtilitiesException
    {
        ...
        while ( !inputFile.EOF() )
        {
            ...
            {
                String entry = Strings.trim( (String)st.nextElement(), " '\"" );
                if ( colTypes[col] == LEFT ) outLine += Strings.leftJustify( entry, colWidths[col] );
                else outLine += Strings.rightJustify( entry, colWidths[col] );
