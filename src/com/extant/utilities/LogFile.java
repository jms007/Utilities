...
public class LogFile
{
	...
	public LogFile(String fileName, boolean append)
	{
		...
		try
		{
			uf = new UsefulFile(logFileName, mode);
		} catch (IOException iox)
		{ // We force a valid log file to avoid throwing an exception here
			File temp;
			try
			{
				temp = File.createTempFile("Log", ".log");
			} catch (IOException iox2)
			{
				Console.println(
						"[LogFile] Unable to use " + fileName + " and unable to createTempfile() ... aborting.");
				return;
			}
			logFileName = temp.getAbsolutePath();
			Console.println(
					"[LogFile] Unable to use " + fileName + ". " + "Created " + logFileName + " for this LogFile.");
			try
			{
				uf = new UsefulFile(logFileName, "w");
	...
	public void log(int level, String msg)
	{
		...
		if (logThis)
		{
			String logEntry = new Julian().toString(timestampFormat) + " [" + Strings.colFormat(level, 2) + "] " + msg;
			if (logToFile)
				uf.writeLine(logEntry);
			if (logToConsole)
			{
				if (timestampConsoleLog)
	...
	public void whereAreWe(int opt, Throwable x)
	{
		...
		if (opt == 0)
			stackSize = stackTraceElements.length;
		else
			stackSize = opt;
	...
	/*
	 * I finally gave up on trying to make this compile and execute correctly under
	 * NetBeans. (See the method fatalError(msg) in VLUtil.) Unfortunaately, this
	 * requires modification of all calls to the original logFatal(msg, new Error()
	 */
	...
	public void logMark(String msg)
	{
		System.out.println("logMark: " + msg);
		whereAreWe(new Error());
	}
...
/*
 * Conventions for assigning logLevels: 0 - 9: Fatal errors; Errors which
 * interrupt processing (Exceptions) 10 - 19: Application entry & exit messages;
 * Major Info; Major trace points; ------------- Normal Log Level -------------
 * 20 - 29: Minor trace points; Minor Info 30 - 39: 40 - 49: 50 - 59: 60 - 69:
 * 70 - 79: 80 - 89: 90: SQL Commands * ------------- Debug Log Level
 * ------------- 91 - 99: Debug output
 *
 * Note that levels > 99 can be used, but they cannot be turned on and off as
 * specific levels.
 *
 * Production applications should normally run with logLevel = NORMAL_LOG_LEVEL.
 * For debugging, use DEBUG_LOG_LEVEL.
 *
 * An application which makes a log file should make a log entry at the start of
 * execution at logLevel APP_ENTRY_EXIT in the following form: [<class-name>]
 * <other useful information> If the log might be analyzed by a program, some
 * attention should be given to ease of parsing the entries, but the primary
 * consideration is to make log entries easily readable by people.
 */
