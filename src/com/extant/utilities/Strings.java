...
implements FilenameFilter
  ...
  public boolean accept( File dir, String name )
    ...
    if ( !patternIsRegex )
      //return ( match( pattern, name ) && dir.isFile() ); // This excludes sub-directories
...
//Console.println( "patternX( " + dosPattern + " ) = " + work );
  ...
  public static String fileSpec( String what, String pathAndFile )
    ...
    if ( what.startsWith( "D" ) )
    {
      ...
      { // PATH (includes drive letter, if any) with trailing separator (does NOT include the filename!)
      if ( p < 0 ) return "";
      ...
      if ( p < 0 )
      {
          if ( pdot < 0 ) return pathAndFile;
          else return pathAndFile.substring( 0, pdot );
      }
      if ( pdot < 0 ) return pathAndFile.substring( p+1 );
      else return pathAndFile.substring( p+1, pdot );
      }
    else if ( what.startsWith( "F" ) )
      { // FILE name, including Extention if any
      if ( p < 0 ) return pathAndFile;
    ...
    {
        if ( !newExtention.startsWith( "." ) ) newExtention = "." + newExtention;
        return Strings.fileSpec( "P", inputFileName ) +
            Strings.fileSpec( "FO", inputFileName ) + newExtention;
    }
  ...
  public static boolean validateChars( String s, String validChars )
    ...
    for (int i=0; i<s.length(); ++i)
      ...
      if ( !validateChar( s.charAt( i ), validChars ) ) return false;
    return true;
    }
  ...
  public static boolean containsAny( String haystack, String needles )
    ...
    for (int i=0; i<needles.length(); ++i)
        if ( validateChar( needles.charAt( i ), haystack ) ) return true;
    return false;
    }
  ...
  public static String rightJustify( String s, int colWidth )
    {
    if ( s.length() >= colWidth ) return s.substring( 0, colWidth );
    String ans = "";
  ...
  public static String center( String s, int colWidth )
    ...
    for (int i=0; i<nSpace; ++i) space += " ";
    String ans = space + s;
    if ( ans.length() > colWidth ) ans = ans.substring(0,ans.length()-1);
    while ( ans.length() < colWidth ) ans += " ";
    return ans;
    }
...
//    return sb.toString();
  ...
  public static String colFormat( String sv, String f )
    ...
    if ( sv.length() > f.length() ) return sv.substring( 0, f.length() );
    String s = sv;
    while ( s.length() < f.length() ) s += " ";
    return s;
    }
  ...
  public static String fixPennies( String s )
    ...
    if ( s1.equals( "" ) ) return "";
    if ( s1.startsWith( "." ) ) s1 = "0" + s1; // for .xx
    if ( s1.startsWith( "-." ) ) s1 = "-0." + s1.substring( 2 ); // for -.xx
    int p1 = s1.indexOf( '.' );
    if ( p1 != s1.lastIndexOf( '.' ) ) return "";
    if ( p1 < 0 ) s1 += ".";
    p1 = s1.indexOf( '.' );
    int decimalDigits = s1.length() - p1 - 1;
    if ( decimalDigits > 2 ) s1 = s1.substring( 0, p1 + 2 );
    //if ( decimalDigits > 2 ) return ""; // mils are not allowed
    //if ( decimalDigits == 2 )  ; // This is the normal case
    if ( decimalDigits == 1 ) s1 += "0";
    if ( decimalDigits == 0 ) s1 += "00";
    int p0 = 0;
    if ( s1.startsWith( "-" ) ) p0 = 1;
  ...
  public static long parsePennies( String s )
    ...
    return ( parseLong( s1.substring( 0, p1 ) ) * 100L )
         + sign * parseLong( s1.substring( p1+1 ) );
    }
  ...
  public static String formatPennies( long pennies, String formatOptions )
    ...
    if ( validateChars( ',', formatOptions ) ) ts = ",";
    if ( validateChars( '<', formatOptions ) ) sign = "<";
    if ( validateChars( '(', formatOptions ) ) sign = "(";
    if ( pennies < 0 )
      {
      negative = true;
...
//Console.println( "three2text( " + n + ", " + tag + ")" );
    String text = "";
    if ( n >= 100 ) text += units[n/100] + " HUNDRED ";
    int t = (n % 100) / 10;
    if ( t == 1 )
    {
        text += teens[n % 10] + " ";
  ...
  public static final String[] teens = new String[]
  ...
  public static boolean isValidFloat( String s )
    ...
    catch (NumberFormatException nfx) { return false; }
    }

  public static float parseFloat( String s )
  throws NumberFormatException
    {
    if ( trim( s, " " ).length() == 0 ) return (float)0.0;
  ...
  public static String preParse( String s, String token, String delimiter )
  throws UtilitiesException // for unmatched delimiters in s
    ...
    for (int i=0; i<s.length(); ++i)
      ...
      if ( inQuote && s.charAt( i ) == token.charAt( 0 ) ) newString += (char)0;
...
//Console.println( "preParse returns " + newString );
  ...
  /*****  First test program *****
  // Use: Strings -n
  // where n is the test number you want to run
  // Defaults to test #8
  
  public static void main( String[] args )
    {
    Clip clip = null;
    try { clip = new Clip( args,  new String[] { "-8" } ); }
    catch ( UtilitiesException ux) { Console.println( ux.getMessage() ); }
    
    Console.println("Running main test");
    if ( clip.isSet( '1' ) )
      {
      Console.println("Running test #1");
      try
        {
        Strings strings = new Strings();
        String[] javaFiles;
        String dir = "E:\\Projects\\Java\\com\\extant\\utilities\\";

        String pattern = "*.java";
        javaFiles = strings.getMatchingFiles( dir, pattern );
        Console.println( "DOS-style: Found " + javaFiles.length + " files matching " + pattern );
        pattern = ".*\\.java";
        javaFiles = strings.getMatchingFilesRegex( dir, pattern );
        Console.println( "regex-style: Found " + javaFiles.length + " files matching " + pattern );
        ...
        String pathAndFile = dir + "example.java";
        Console.println( "For: " + pathAndFile + "\n" +
                         "   PATH = " + Strings.fileSpec( "PATH", pathAndFile ) + "\n" +
                         "   path = " + Strings.fileSpec( "path", pathAndFile ) + "\n" +
                         "   FILE = " + Strings.fileSpec( "FILE", pathAndFile ) + "\n" +
                         "   FO   = " + Strings.fileSpec( "FO"  , pathAndFile ) + "\n" +
                         "   EXT  = " + Strings.fileSpec( "EXT" , pathAndFile ) );
        pathAndFile = dir + "example";
        Console.println( "For: " + pathAndFile + "\n" +
                         "   PATH = " + Strings.fileSpec( "PATH", pathAndFile ) + "\n" +
                         "   path = " + Strings.fileSpec( "path", pathAndFile ) + "\n" +
                         "   FILE = " + Strings.fileSpec( "FILE", pathAndFile ) + "\n" +
                         "   FO   = " + Strings.fileSpec( "FO"  , pathAndFile ) + "\n" +
                         "   EXT  = " + Strings.fileSpec( "EXT" , pathAndFile ) + "\n" +
                         "   ..   = " + Strings.fileSpec( ".."  , pathAndFile ) );
        Console.println( "For " + dir + "\n" +
                         "   PATH = " + Strings.fileSpec( "PATH", dir ) + "\n" +
                         "   path = " + Strings.fileSpec( "path", pathAndFile ) + "\n" +
                         "   FILE = " + Strings.fileSpec( "FILE", dir ) + "\n" +
                         "   FO   = " + Strings.fileSpec( "FO"  , dir ) + "\n" +
                         "   EXT  = " + Strings.fileSpec( "EXT" , dir ) + "\n" +
                         "   ..   = " + Strings.fileSpec( ".."  , dir ) );
        }
      catch (Exception x)
        {
    ...
    if ( clip.isSet( '2' ) )
      {
      Console.println( "Running test #2 ..." );
      long t = 12345678L;
      Console.println( t + " -> " + formatPennies( t ) );
      t = 123456789L;
      Console.println( t + " -> " + formatPennies( t ) );
      t = 1234567899L;
      Console.println( t + " -> " + formatPennies( t ) );
      ...
      String[] pennies = new String[] { "123.45", "123,456.78", "123", "123.4", "12x.45" };
      for (int i=0; i<pennies.length; ++i )
          Console.println( pennies[i] + " --> " + fixPennies( pennies[i] ) +
            "  valid=" + isValidPennies( pennies[i] ) );
      }
    if ( clip.isSet( '3' ) )
      {
          Console.println("Running test #3");
      String test[] = new String[] {
          "1,2,3,4",
          "1,2,3,4,'this is easy.',5,6,7,8",
          "1,2,3,4,'this is hard, right?',5,6,7,8",
          "1,2,3,4,'and this is even worse!,5,6,7,8" };
      int i=-1;
      try
        {
        for (i=0; i<test.length; ++i)
          {
          Console.println( "Test String: " + test[i] );
          Console.print( "elements: " );
          StringTokenizer st = new StringTokenizer( preParse(test[i], ",", "'" ), "," );
          while ( st.hasMoreElements() )
            {
            Console.print( "  <" + postParse( (String)st.nextElement(), "," ) + ">" );
            }
          Console.println( "" );
          }
        }
        ...
        {
        Console.println( ux.getMessage() + " in line " + i + ":" );
    ...
    if ( clip.isSet( '4' ) )
    {
        Console.println("Running test #4");
        Console.println( "'" + Strings.format( 12345, "####0" ) + "'" );
        Console.println( "'" + Strings.format( 1,     "####0" ) + "'" );
        Console.println( "'" + Strings.colFormat( 1, 2 ) + "'" );
    }
    if ( clip.isSet( '5' ) )
    {
        Console.println("Running test #5");
        String[] test = new String[]
        { "1\\n12\\n123\\n12345\\n1234\\n"
        ...
        for (int i=0; i<test.length; ++i)
            Console.println( "The longest line in test[" + i + "] is:\n'" + Strings.findLongestLine( test[i] ) + "'" );
    }
    if ( clip.isSet( '6' ) )
    {
        Console.println("Running test #6");
        String test[] = new String[]
        { "a string with one #oldThing# to be replaced"
        , "multiple #oldThing# fields that should change to #oldThing#"
        };
        for (int i=0; i<test.length; ++i)
            Console.println( test[i] + " --> " + replace( test[i], "#oldThing#", "longNewThing" ) );
    }

    if ( clip.isSet( '7' ) )
    {
        Console.println("Running test #7");
        long values[] = new long[] { 99L, 100L, 1000L, 10000L, 100000L, 1000000L, 10000000L,
            100000000L, 1000000000L };
        for (int i=0; i<values.length; ++i)
            Console.println( formatPennies( values[i], "," ) + " --> " +
                pennies2Text( values[i] ) );
    }
    if ( clip.isSet( '8' ) )
    {
        Console.println("Running test #8");
        String s1[] = new String[] { "12345", "655*", "655*", "*.*" };
        String s2[] = new String[] { "12345", "655" , "655x", "file.ext" };
        for (int i=0; i<s1.length; ++i)
            Console.println( s2[i] + " match " + s1[i] + " = " + match( s1[i], s2[i] ) );
    }
